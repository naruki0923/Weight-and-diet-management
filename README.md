...
| 送る文字 | 動作 |
|---|---|
| `TDEE` | 今の身体データで BMR / TDEE / 推奨カロリーを表示 |
| `TDEEを目標に` | 推奨カロリーを目標カロリーとして設定 |
| `活動レベル` | レベル一覧を表示 |
| `活動レベル 3` | 活動レベルを設定して再計算 |
| `年齢 20` / `身長 168` / `性別 男` | 身体データを設定して再計算 |
| `目標区分 増量` | 増量 / 維持 / 減量 を設定 |
| `カロリー調整 500` | TDEE への増減を直接指定 |
| `55.5`（数字だけ） | 体重を記録し、その体重で目標カロリーを再計算 |

初回は `年齢` `身長` `性別` `活動レベル` `目標区分` を一度送っておく（設定シートに保存される）。
